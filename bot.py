...
TEXTO_START = (
    ...
    "🚀 <b>¿Cómo empezar? Usa el menú interactivo de botones aquí abajo o escribe los comandos ( Solo USUARIOS VIP ):</b>\n"
...
TEXTO_BPAY = (
    ...
    "• Provincial (BBVA): 1.5% comisión, se recomienda dejar un margen fijo de 3$ a 5$ en la cuenta USD para evitar errores.\n\n"
...
TEXTO_GPAY = (
    ...
    "• BDV Tarjeta Internacional: Restar 2.5%\n"
    "• Banco del Tesoro: Restar 2.5%\n"
    "• Provincial (BBVA): 1.5% comisión, pero se recomienda dejar un margen fijo de 3$ a 5$ en la cuenta para evitar errores.\n\n"
...
TEXTO_ANUNCIO_VIP = (
    f"<blockquote>{e('SIRENA', '🤚🏽')} <b>ANUNCIO OFICIAL: ACTUALIZACIÓN Y MODELO DE SOSTENIBILIDAD</b></blockquote>\n\n"
    f"Estimada comunidad,\n\n"
    f"Para garantizar el funcionamiento 24/7 de nuestro bot, mantener la rapidez de respuesta "
    f"y asegurar la actualización constante de los monitores en servidores dedicados, hemos "
    f"implementado un modelo de suscripción <b>VIP a bajo costo</b> para las funciones avanzadas.\n\n"
    f"<blockquote>{e('GUIAS', '🤚🏽')} <b>GUIAS PRACTICAS GRATUITAS (Para todos):</b></blockquote>\n"
    f"• Usuarios comunes <b> 📜Regla de Oro📜</b>\n"
    f"• <code>/bp</code> 🟠 <b>BPay</b> 🟠\n"
    f"• <code>/gp</code> 🔵 <b>GPay</b> 🔵\n"
    f"• ⚙️ <b>Soporte</b>\n"
    f"• 🤖 <b>Ia Consulta</b>\n\n"
    f"<blockquote>{e('ESCUDO', '🛡️')} <b>MÓDULOS EXCLUSIVOS VIP:</b></blockquote>\n"
    f"{e('VERDE', '🟢')} <b>P2P-USDT</b> {e('ROJO', '🔴')}\n"
    f"• 📊 <b>Intervencion</b> 📊\n"
    f"• 📟 <b>Calculadora</b>\n"
    f"• 📊 <b>Arbitraje & Reposición</b> 📊\n\n"
    f"<blockquote>{e('MONITOR', '🖥️')} <b>MONITORES: ADMINISTRADORES VIP EN GRUPO, USUARIOS VIP CHAT INTERNO {e('ROBOTICO', '💫')} BOT</b></blockquote>\n"
    f"• <code>/brecha</code>\n"
    f"• <code>/zinli</code>\n"
    f"• <code>/i</code>\n"
    f"• <code>/p</code>\n"
    f"• <code>/tasa</code>\n\n"
    f"{e('BOMBILLA', '😶‍🌫️')} <i>Uso optimizado en grupos para Administradores VIP o en chat privado para todos.</i>\n\n"
    f"💎 <b>TARIFA ACCESIBLE:</b>\n"
    f"• <b>Suscripción VIP {e('RAYITO', '🚿')}:</b> <b>2</b> {e('USDT', '🪙')} / <b>Quincenal</b> (o Equivalente en Bs).\n"
    f"<blockquote><b>ACTIVACIÓN CHAT PRIVADO BOT {e('ROBOTICO', '💫')}:</b></blockquote> Toca en: 1️⃣ <b>Enviar Solicitud VIP</b> , 2️⃣ ⚙️ <b>Soporte</b> para ver las cuentas de pago y 3️⃣ <b>Envíar comprobante.</b>\n\n"
    f"¡Gracias por su constante apoyo para mantener este proyecto activo y en evolución! {e('BOOM', '🟢')}"
)
...
def obtener_datos_bcv_validos():
    ...
    try:
        if 'r' in globals() and r:
            ...
            fecha_ultima_rotacion = str(datos.get("fecha_ultima_rotacion", "")).strip()

            es_fin_de_semana = hora_ve.weekday() in [5, 6]

            # Evaluamos si hay tasa guardada y no hemos rotado hoy
            if tasa_manana_val > 0 and fecha_ultima_rotacion != fecha_hoy_sistema and not es_fin_de_semana:
                datos["tasa_anterior"] = float(datos.get("tasa_hoy", 0.0))
                datos["fecha_anterior"] = datos.get("fecha_hoy", "")

                datos["tasa_hoy"] = tasa_manana_val
                datos["fecha_hoy"] = fecha_manana_guardada if fecha_manana_guardada else fecha_hoy_sistema

                # Limpiamos para el raspado de la tarde
                datos["tasa_manana"] = 0.0
                datos["fecha_manana"] = ""
                datos["fecha_ultima_rotacion"] = fecha_hoy_sistema

                r.set("bcv_datos_v14", json.dumps(datos))
                print(f"🔥 [ROTACIÓN NOCTURNA] Tasa de mañana ({tasa_manana_val}) promovida a Tasa Hoy para {fecha_hoy_sistema}")
...
def obtener_tasa_binance_p2p_bdv(tipo_operacion, monto_bs):
    ...
    payload = {
        ...
        "payTypes": ["BancoDeVenezuela"],  # Filtro oficial BDV
        "filterType": "tradable",
        "additionalKycVerifyFilter": 0,
        "periods": []
    }
