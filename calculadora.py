...
def registrar_calculadora(bot, obtener_cache_func, funcion_menu_principal):
    """
    Registra la lógica de la calculadora BCV + 0.5% en chat privado.
    """

    def solicitar_monto(call):
        """Paso 1: Se activa al presionar el botón 'Calculadora'"""
        if call.message.chat.type != 'private':
            bot.answer_callback_query(call.id, "⚠️ La calculadora solo funciona en chat privado.")
            return
    ...
    def procesar_calculo(message):
        """Paso 2: Procesa el monto escrito por el usuario"""
        ...
        try:
            ...
            bot.send_message(
                message.chat.id,
                "⚠️ *Monto inválido.* Escribe solo números (Ejemplo: `15` o `20.5`).",
                parse_mode="Markdown",
                reply_markup=markup
            )
            return
        ...
        tasa_con_intervencion = tasa_bcv * 1.005  # BCV + 0.5%
